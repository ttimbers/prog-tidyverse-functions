...
And then remind ourselves, what a function looks like in R!

In R, we define and name a function via:
...
It turns out no! Why not? The unquoted column names 
trip R up here! Let's learn more about why, and then what we can do to handle 
this!
...
### Solving our problem from the beginning

Can we use what we learned now to fix our function from earlier? Yes! We need to
add the `{{` operator around the unquoted column name data variables in our 
function:
